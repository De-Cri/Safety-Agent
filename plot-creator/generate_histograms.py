...
def _render_calendar_heatmap(data: ChartData) -> str:
    ...
    for w in range(weeks):
        ...
        if d_w.month != prev_month:
            ...
            month_labels.append(d_w.strftime("%b %Y"))
...
def _render_heatmap_grid(data: ChartData) -> str:
    """Matrice 7 giorni × 24 ore: cella = n. eventi."""
    import matplotlib
    matplotlib.use('Agg')
    import matplotlib.pyplot as plt
    import numpy as np
    ...
    grid = np.array(matrix)  # shape (7, 24)
    fig, ax = plt.subplots(figsize=(14, 4))
    im = ax.imshow(grid, aspect="auto", cmap="YlOrRd", interpolation="nearest")

    ax.set_yticks(range(7))
    ax.set_yticklabels(["Lun", "Mar", "Mer", "Gio", "Ven", "Sab", "Dom"], fontsize=9)
